...
| S-2 | 신호 Raise 지점이 3곳뿐(적용/삽입/주입/흡인/사정/전달/진입 미구현) | `interaction-signal-integration-spec.md §0,§2` + 코드 실측 | 핵심 처치 게이트가 통과 불가 | (진행) 구역 진입(`enter_*`/`arrive_*`) 계측 완료(`ScenarioTriggerZone._raiseSignalsOnEnter`). 부착형 적용/착용(`apply_*`/`wear_glove`) 계측 완료(`IItemUseTarget` 브리지 + `ApplySignal`, 제안서 `done/2026-06-25-item-use-target-signal/`). 주입/흡인/앤부(`push_*`/`suction_*`/`start_ambu`)는 Item Use Signals 매핑으로 계측 완료(2026-06-25). 잔여: 정맥 삽입/제거(`insert_iv_*`/`remove_*`), 사정·전달·신체부위/장비 클릭 |
