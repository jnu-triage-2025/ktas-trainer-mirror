...
| S-2 | 신호 Raise 지점이 3곳뿐(적용/삽입/주입/흡인/사정/전달/진입 미구현) | `interaction-signal-integration-spec.md §0,§2` + 코드 실측 | 핵심 처치 게이트가 통과 불가 | (진행) 구역 진입(`enter_*`/`arrive_*`)은 2026-06-25 `ScenarioTriggerZone._raiseSignalsOnEnter`로 계측 완료. 적용/주입(`apply_*`/`push_*`)은 Feature Proposal(`scheduled/2026-06-25-item-use-target-signal/`)로 분리. 신체부위/장비 클릭·사정·전달은 잔여 |
