...
humanize-korean 도구를 실행할 수 없는 경우, **작업을 중단하지 않는다.** 다음 절차를 따른다:

1. **`python3` 런타임 확인:** `python3 --version`으로 설치 여부를 확인한다. 없으면 환경 설정을 점검한다.
2. **그래도 실패하면** (스크립트 오류, 참조 파일 누락 등): 결과물을 그대로 전달하되 아래 두 경고를 반드시 포함한다.

**사용자 경고 (채팅 출력):**
```
⚠️ humanize-korean 도구를 거치지 않았습니다.
```

**생성 텍스트 내 경고 (문서·이슈 본문 최상단):**
```markdown
> ⚠️ 이 텍스트는 AI 문체 순화 처리를 거치지 않았습니다.
```

**코드 주석 예외:** 코드 주석(XML documentation comments 등)에는 경고 텍스트를 삽입하지 않는다. 사용자 경고만 표시한다.
