...
## 커밋 시 unity-merge 툴에 의한 uuid 추적 데이터 생성

커밋 시에 unity-merge 툴에 의해 uuid 추적 데이터가 `.unity-merge/` 디렉토리에 생성된다. 이 데이터는 계속해서 git에 의해 추적되는 것이 의도이며, 어떤 수정이 존재할 때 그 수정과 함께 커밋되어야 한다. (Unity .meta 파일과 비슷한 유형으로 취급한다.) 따라서 `.unity-merge/` 디렉토리가 변화한다면 그것을 의도적으로 unstage하거나 삭제하지 말아야 한다. 하지만 이러한 내용을 커밋에 포함시켰다는 서술을 커밋 등에 명시적으로 작성할 필요는 없다. (커밋 메시지에 `uuid 추적 데이터`라는 문구를 포함시키는 것은 불필요하다.)
