...
namespace MultiplayerInfrastructure.Audio
{
  ...
  public static class VoiceChatSettings
  {
    ...
    private const string MutedPlayersKey = "voiceChat.mutedPlayers";
    ...
    public static bool IsPlayerMuted(string identifier) => LoadMutedPlayers().Contains(identifier ?? string.Empty);
    public static void SetPlayerMuted(string identifier, bool muted)
    {
      if (string.IsNullOrWhiteSpace(identifier)) return;
      var identifiers = LoadMutedPlayers();
      if (muted) identifiers.Add(identifier); else identifiers.Remove(identifier);
      PlayerPrefs.SetString(MutedPlayersKey, string.Join("\n", identifiers));
    ...
    public static void ClearStoredValues()
    {
      ...
      PlayerPrefs.DeleteKey(SensitivityKey); PlayerPrefs.DeleteKey(MutedPlayersKey);
      PlayerPrefs.Save(); Changed?.Invoke();
    }
    ...
    private static HashSet<string> LoadMutedPlayers()
    {
      var result = new HashSet<string>(StringComparer.Ordinal);
      string serialized = PlayerPrefs.GetString(MutedPlayersKey, string.Empty);
      foreach (string identifier in serialized.Split(new[] { '\n' }, StringSplitOptions.RemoveEmptyEntries)) result.Add(identifier);
      return result;
    }
  }
}
