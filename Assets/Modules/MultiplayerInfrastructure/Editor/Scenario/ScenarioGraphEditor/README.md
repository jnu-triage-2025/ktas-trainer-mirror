...
## UI 구성 요소
- 상단 툴바: `File`, `Edit`, `View`, 노드 추가, 검색 및 그래프 메타데이터 필드.
- 툴바 메뉴: `File`(New/Open/Save 등 — `Open Recent` 하위 메뉴 포함), `Edit`(`Relocate Nodes`), `View`(`Reset Minimap`).
...
- 목록은 프로젝트 로컬 캐시인 `Library/ScenarioGraphEditor/refs.json` 에 저장됩니다. 버전 관리에는 포함되지 않지만 에디터를 재시작해도 유지되며, 파일 삭제 시 목록에서도 자동 제외됩니다.
- 하위 메뉴 하단의 `Clear Recent` 로 목록을 비울 수 있습니다.
- 현재 파일이 존재하면 `File` 메뉴의 `Reveal in Finder`(macOS), `Show in File Explorer`(Windows), `Show in File Manager`(Linux)로 운영체제 파일 관리자에서 파일을 선택해 표시할 수 있습니다.
...
- 각 연결의 대상은 원본보다 오른쪽 레이어에 배치됩니다. 좌표 압축 단계에서는 부모가 자식 묶음의 세로 중심에 오도록 정렬합니다.
- 모든 진입 노드는 `x = 0`에서 시작합니다. 여러 레이어를 건너뛰는 간선은 내부 dummy vertex 체인으로 정규화해, 일반 간선과 동일하게 교차 최소화합니다.
- Choice·Parallel·Quiz 분기는 실제 출력 포트의 위→아래 순서를 보존합니다. 각 sweep은 교차 수를 최우선으로 비교하고, 교차 수가 같으면 총 세로 변위가 더 작은 순서를 선택합니다. 인접 노드 교환에도 같은 기준을 적용해 수평 연결이 많은 결과를 유지합니다.

## 미니맵

- 미니맵은 Unity의 실험적 `GraphView.MiniMap` 대신 시나리오 그래프 전용 렌더러와 입력 처리를 사용합니다.
- 그래프의 가로·세로 비율을 유지해 축별 왜곡 없이 표시하며, 남는 축에는 여백을 둡니다.
- 현재 그래프 뷰포트는 반투명 흰색 영역과 얇은 흰색 테두리로 표시됩니다.
- 미니맵 위치와 크기는 Open Recent 캐시(`Library/ScenarioGraphEditor/refs.json`)에 파일별로 함께 저장되며, 같은 파일을 다시 열 때 복원됩니다.
- 미니맵 헤더를 드래그하면 그래프 영역 안에서 위치를 옮길 수 있습니다.
- 미니맵 테두리와 모서리를 드래그하면 크기를 조정할 수 있습니다.
- 미니맵 본문을 클릭하거나 드래그하면 현재 그래프 뷰포트가 해당 위치로 이동합니다.
- 상단 `View → Reset Minimap`을 선택하면 위치와 크기가 기본값으로 돌아갑니다.
- 순환 연결은 화면에 유지되지만 레이어 계산에서는 feedback edge로 처리합니다. 기본 간격은 가로 약 420px, 세로 약 240px입니다. 복잡한 그래프는 자동 배치 후 수동 위치를 조정해 저장할 수 있습니다.
