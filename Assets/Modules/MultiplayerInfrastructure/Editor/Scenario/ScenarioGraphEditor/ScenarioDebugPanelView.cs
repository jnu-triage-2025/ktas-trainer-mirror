...
namespace MultiplayerInfrastructure.Editor
{
  ...
  public sealed class ScenarioDebugPanelView : VisualElement
  {
    ...
    private VisualElement BuildGroup(string nodeId, List<ScenarioGraphDiagnostics.DiagnosticItem> groupItems)
    {
      ...
      if (errorCount > 0)
        groupHeader.Add(BuildBadge($"✕ {errorCount}", ColorBadgeError));
      if (warnCount > 0)
        groupHeader.Add(BuildBadge($"⚠ {warnCount}", ColorBadgeWarn));
      if (infoCount > 0)
        groupHeader.Add(BuildBadge($"ℹ {infoCount}", ColorBadgeInfo));
    ...
    {
      if (string.IsNullOrEmpty(_selectedItemMessage))
        return false;
