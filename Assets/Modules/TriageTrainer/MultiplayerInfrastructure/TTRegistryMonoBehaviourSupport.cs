...
{
  /// <summary>
  /// TriageTrainer 모듈의 레지스트리 초기화 및 런타임 검증을 담당하는 MonoBehaviour입니다.
  ///
  /// Awake() 에서 아이템 정의를 Registry에 등록하고,
  /// 등록된 모든 아이템에 대해 필요한 리소스(아이콘 스프라이트, 3D 모델 프리팹)가
  /// Resources 폴더 내에 존재하는지 검증합니다.
  /// </summary>
  public class TTRegistryMonoBehaviourSupport : MonoBehaviour
  {
    ...
    public void ValidateItemResources()
    {
      ...
      {
        Debug.LogWarning("[TTRegistryMonoBehaviourSupport] 등록된 아이템이 없습니다.");
        return;
      }
      ...
      foreach (string id in registeredItems.Keys)
      {
        ...
        {
          Debug.LogWarning(
            $"[TTRegistryMonoBehaviourSupport] 아이콘 스프라이트 누락 " +
        ...
        {
          Debug.LogWarning(
            $"[TTRegistryMonoBehaviourSupport] 3D 모델 프리팹 누락 " +
      ...
      {
        Debug.Log(
          $"[TTRegistryMonoBehaviourSupport] 아이템 리소스 검증 완료 — " +
