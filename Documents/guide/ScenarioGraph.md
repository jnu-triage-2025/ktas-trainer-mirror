...
플레이어에게 "어떻게 할지"를 직접 고르게 하는 노드입니다. 여러 개의 선택지(`options`)를 보여주고, 플레이어가 그중 하나를 고르면 그 선택지에 적힌 `nextNodeIdentifier`로 이동합니다. Dialogue 노드와 달리 진행 방향은 선택지가 결정하므로, 노드 자체의 `nextIdentifier`는 정상 선택에는 쓰이지 않습니다. 이 값은 선택지를 표시하지 못했거나 180초 동안 응답이 없을 때의 복구 경로로만 쓰입니다. 비워 두면 다음 노드가 지정된 첫 번째 선택지의 경로로 복구되는데, 그 경로가 오답 재시도 대사를 거쳐 같은 선택지로 되돌아오면 응답하지 않는 플레이어는 영원히 그 자리를 맴돌고 나머지 인원은 다음 합류점에서 그 플레이어를 기다리게 됩니다. 오답 선택지가 되돌아오는 구조라면 반드시 `nextIdentifier`에 앞으로 나아가는 노드(예: 정답 이후 노드)를 지정하세요. 복구로 진행한 선택은 정답으로 평가되지 않습니다.
...
| `nextIdentifier` | `string` (nullable) | 정상 선택에는 쓰이지 않고, 선택지 표시 실패·응답 없음(180초)의 복구 경로로만 쓰입니다. `null`이면 다음 노드가 지정된 첫 선택지의 경로로 복구합니다. 오답 선택지가 같은 선택지로 되돌아오는 구조에서는 반드시 지정하세요 | `N006_3` |
...
| `waitForResolution` | `bool` | 참이면 신호가 resolve될 때까지 다음 노드 진행을 막습니다(기본값 true). 대기 상한은 `waitTimeoutSeconds`이며, 미지정/0 이하이면 Validator 게이트와 같은 180초 복구 상한을 적용합니다 | `true` |
| `nextIdentifier` | `string` | 다음 진행 노드의 식별자 | `next-node-identifier` |
...
합류는 서버만 해제하며, 접속 중인 참여자가 유효한 작업을 하는 동안에는 시간으로 해제하지 않는다.
대신 피어별 대기(게이트·대화·선택지·내부 신호 등록)는 모두 복구 상한으로 끝나므로, 각 피어는 반드시
합류점에 도달한다. 흐름이 먼저 끝난 피어는 종료 시 서버에 이탈을 알려 배리어에서 빠진다.
알 수 없는 원인으로 완료 보고가 영영 오지 않는 경우에만 최초 완료 보고부터 30분 뒤 최후 상한으로 해제한다.
수동 진입으로 합류한 피어는 기존 참여자의 합류를 다시 잠그지 않는다.

`ByRole` 병렬의 역할 배정도 서버가 내려 준다. 서버는 최초 요청부터 10초 동안 접속 중인 참여자 전원의
역할 등록을 기다리고, 그 뒤에는 역할 없는 참여자를 제외하고 배정한다. 로스터 자체를 만들 수 없으면
(중복 역할 등) 모든 분기를 건너뛰는 배정을 내려 준다. 클라이언트는 180초 안에 배정을 받지 못하면
미수행을 기록하고 자기 로스터로 배정한다. 시작 명령은 역할 없는 대상을 포함한 시작을 거부한다.
