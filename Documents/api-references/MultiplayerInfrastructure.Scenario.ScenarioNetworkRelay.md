...
- 관련 제안서: `Agents/Proposals/완료/2026-06-24-scenario-parallel-execution/server-authoritative-execution-spec.md`

## 6. 인터렉션 가시성·엔티티 태그 중계 (2026-09-06)

인터렉션 레지스트리의 가시성 오버라이드와 플레이어가 아닌 엔티티의 태그는 신호와 같은 방식으로 서버 권위 중계된다.

| 메서드 | 역할 |
|---|---|
| `ApplyInteractionVisibilityAuthoritative(...)` | 서버면 `InteractionVisibilityState`에 기록 후 `ObserversMirrorInteractionVisibility`로 미러링, 클라이언트면 `CmdApplyInteractionVisibility`로 위임 |
| `PublishEntityTags(entityIdentifier, tags)` | 소유 플레이어가 없는 엔티티의 태그를 `ObserversMirrorEntityTags`로 미러링 |
| `SendInteractionStateSnapshot(connection)` | 늦게 접속한 피어에 오버라이드 스냅샷(`TargetMirrorInteractionVisibilitySnapshot`)과 엔티티 태그 스냅샷(`TargetMirrorEntityTagsSnapshot`)을 전달. 신호 스냅샷 요청(`CmdRequestRaisedSignalSnapshot`)과 늦은 접속 경로에서 호출된다 |

중계기가 없으면 로컬 폴백으로 기록만 하므로 오프라인·단독 실행에서도 동작한다.
