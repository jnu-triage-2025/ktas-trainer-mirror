# 아이템 Authoring/구현 통합 가이드

이 문서는 기존 `item-authoring.md`와 `item-implementation-guide.md`를 통합한 단일 기준 문서입니다.
...
- **씬 배치 아이템은 authored entity ID를 가져야 합니다.** `SceneItemPlacement`는 `OnValidate()`에서 `_entityIdentifier`를 자동 보정하며, 이 값이 서버와 모든 클라이언트에서 동일한 월드 아이템 식별자가 됩니다.
- **클라이언트는 씬 배치 아이템을 직접 확정하지 않습니다.** 접속 후 자신의 `PlayerController`가 서버의 현재 `ItemObject` 스냅샷을 받아 로컬 월드를 재구성합니다. 따라서 클라이언트 씬 자산의 `_entityIdentifier`가 달라도, 실제 인게임 식별자는 서버가 보낸 값이 우선합니다.
...
- **월드 드롭 시:** `player.TryDropItemInFront(item)`는 로컬에서 바로 `ItemObject.Spawn`하지 않습니다. 소유 클라이언트가 서버에 드롭을 요청하면, 서버가 `item:{guid}` 형식의 entity ID를 발급하고 모든 관전자에게 동일한 ID로 스폰을 전파합니다.
- **월드 픽업 시:** `LootableItemInteractHandler`는 가능하면 `player.TryPickupWorldItem(entityIdentifier)` 경로를 사용합니다. 서버가 거리/중복/소유권을 검증한 뒤 승인하면 대상 클라이언트만 인벤토리에 삽입하고, 성공 확인 전까지는 서버가 제거된 월드 아이템 스냅샷을 보관합니다. 삽입 실패 시 같은 entity ID로 롤백 스폰됩니다.
