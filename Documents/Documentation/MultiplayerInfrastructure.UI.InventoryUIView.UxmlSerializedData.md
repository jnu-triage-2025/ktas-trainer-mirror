# <a id="MultiplayerInfrastructure_UI_InventoryUIView_UxmlSerializedData"></a> Class InventoryUIView.UxmlSerializedData

Namespace: [MultiplayerInfrastructure.UI](MultiplayerInfrastructure.UI.md)  
Assembly: Assembly\-CSharp.dll  

```csharp
[Serializable]
public class InventoryUIView.UxmlSerializedData : VisualElement.UxmlSerializedData
```

#### Inheritance

object ← 
UxmlSerializedData ← 
VisualElement.UxmlSerializedData ← 
[InventoryUIView.UxmlSerializedData](MultiplayerInfrastructure.UI.InventoryUIView.UxmlSerializedData.md)

## Methods

### <a id="MultiplayerInfrastructure_UI_InventoryUIView_UxmlSerializedData_CreateInstance"></a> CreateInstance\(\)

<p>
Returns an instance of the declaring element.
</p>

```csharp
public override object CreateInstance()
```

#### Returns

 object

<p>The new instance of the declaring element.</p>

