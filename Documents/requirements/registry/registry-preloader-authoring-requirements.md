...
- 프리로더는 ProblemSet 사전 로드 항목을 지원해야 하며, 등록 실패 시 경고 로그로 운영자가 원인을 추적할 수 있어야 한다.
...
- 구현 클래스는 `RegistryPreloaderController`이며 `Awake()`에서 Preload 메서드를 순차 호출한다.
...
- ProblemSet은 `RegistryPreloadProblemSetSO`의 `problemSetRegistryRequirements`를 통해 `Registry.PreloadProblemSet(...)` 경로로 로드한다.
