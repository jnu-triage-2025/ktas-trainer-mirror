...
- `nurse_d`에게 퀘스트 발행 (`Quest_IV_Line_PatientA`)
  ...
  - 퀘스트 발행과 함께 다음 처리 수행:
    ...
    3. 2단계 — 생리식염수 활성화(환자가 누워있는 침대의 Attachment에 생리식염수 걸기)
      ...
      - 퀘스트 마크: `bed_a` / `hang_normal_saline` (2026-08-28 추가: "좌측 팔에 생리식염수 연결하기" 목표가 활성 상태인 동안, 아직 생리식염수를 걸지 않았다면 이 Interaction에 마크가 표시된다. 이미 걸어 두었다면 이 Interaction은 노출되지 않으므로 마크도 자연히 사라지고, 3단계 "생리식염수 연결" Interaction 쪽 마크로 넘어간다.)
    ...
    7. 2단계 — 플라즈마 솔루션 활성화(환자가 누워있는 침대의 Attachment에 플라즈마 솔루션 걸기)
      ...
      - 퀘스트 마크: `bed_a` / `hang_plasma_solution` (2026-08-28 추가: "우측 팔에 플라즈마 솔루션 연결하기" 목표가 활성 상태인 동안, 아직 플라즈마 솔루션을 걸지 않았다면 이 Interaction에 마크가 표시된다. 이미 걸어 두었다면 이 Interaction은 노출되지 않으므로 마크도 자연히 사라지고, 3단계 "플라즈마 솔루션 연결" Interaction 쪽 마크로 넘어간다.)
...
#### CPR 수행자 위치 고정과 디버그 Escape 게임룰

- 가슴압박 상호작용을 수행한 플레이어는 상호작용을 발행한 환자 엔티티를 기준으로 위치를 고정한다. CPR 연출이 유지되는 동안 플레이어의 X·Z 위치와 Y 회전을 환자에 맞추고, 일반 이동·점프·시점 회전 입력을 무효화한다.
- 정상 CPR 종료 이벤트가 도착하면 수행자 애니메이션을 Idle 상태로 되돌리고 `MinecraftBoatLikeControl`의 조작 해제와 같은 앵커 해제 경로로 위치 고정을 종료한다.
...
- `cpr_1st_cycle`
  ...
  - 2026-08-28 확정: 이 진입점의 준비 체인은 아래 네 가지를 함께 수행한다. 처치 표현만 켜면 화면은 맞아 보이지만 후속 상호작용이 조용히 실패하기 때문에, 표현과 실제 상태를 반드시 함께 맞춘다.
    1. **심정지 이전의 기술 상태 복원**: 준비 이벤트가 PEA 상태를 기록하므로, 활력징후·의식·피부 등 기술 상태를 되돌리는 `PatientMedicalStatePreset` 노드를 준비 이벤트보다 **먼저** 실행한다. 순서가 뒤바뀌면 프리셋이 심정지 플래그를 다시 지운다.
    2. **처치 상태값을 실제 처치 식별자로 기록**: 단계 요약 식별자(`initial_assessment_complete` 등)만 넣으면 물품 적용 경로가 중복 적용을 막지 못해서, 이미 끝난 경추 고정이나 거즈 지혈을 다시 수행할 수 있다. 따라서 실제 처치 경로가 기록하는 식별자(경추 고정기, 조립된 양커 팁, 기관내관 삽관 완료, 플라스터 고정, T-piece 장착, 산소 연결, 거즈와 거즈 고정)를 함께 기록한다. 첫 맥박 확인 역시 완료 상태로 기록한다.
    3. **장비 설치와 라인 연결 복원**: 벽면 흡인기와 산소 유량계를 설치 상태로 두고, T-piece와 유량계 사이의 산소 라인, 좌측 생리식염수와 우측 플라즈마 솔루션의 정맥로 연결, Level 1 rapid infuser의 수액·혈액백 보유 상태와 환자 C-line 연결을 서버 권위로 되돌린다. 우측 정맥로를 이으려면 침대 수액걸이에 플라즈마 솔루션이 걸려 있어야 하므로, 생리식염수와 같은 방식으로 즉시 설치하는 경로를 사용한다.
    4. **CPR 1주기 상호작용만 개방**: 일반 진행의 `activate_patient_a_arrest_actions`가 여는 두 플래그 가운데 CPR 1주기 처치 동작만 올리고, 이미 끝난 첫 맥박 확인은 열지 않는다. 함께 열면 CPR 1주기 목표와 지나간 단계의 맥박 확인 메뉴가 동시에 노출된다.
  - 제세동기 카트는 정박 지점 **위**가 아니라 정박 판정 거리 바로 밖에 배치한다. 정박 지점에 그대로 올려 두면 도달 신호가 즉시 발생해서, CPR 1주기의 "카트를 환자 옆으로 옮기기" 목표가 아무 행동 없이 통과한다. 준비 체인이 그 도달 신호를 직접 발생시키는 방식도 같은 이유로 사용하지 않는다.
  - 같은 세션에서 앞 단계로 되돌아가 다시 진입할 수 있으므로, 한 번만 수행할 수 있도록 설정된 단계별 상호작용의 "이미 수행함" 표시를 준비 체인이 해제한다. 해제 대상에는 환자에게 붙은 상호작용뿐 아니라 제세동 카트의 "제세동기 담당 역할 부여받기"도 포함한다. 이 상호작용을 빠뜨리면 CPR 2주기의 진행 게이트가 그 신호를 영원히 기다린다. 다만 그 상호작용의 노출 기준값은 건드리지 않는다. 노출 판정은 플레이어별 퀘스트 상태 플래그가 계속 담당해야 하며, 기준값을 함께 켜면 단계 밖 상호작용이 열린다.
...
- scen-a에서 문구가 길어진 자동 진행 대사 두 개의 `autoAdvanceSeconds`를 늘렸다. 대화창 타이핑 속도(0.05초/자) 기준으로
  `D005`(113자, 역할 이름 치환 후 약 60자)는 2초에서 6초로, `D_START_BROADCAST`(73자)는 3초에서 5초로 바꿨다.
  자동 진행 루틴은 타이핑 완료를 기다리지 않으므로, 이전 값으로는 문장이 끝까지 표시되기 전에 다음 노드로 넘어갔다.
