      ...
      - Interaction 활성화 시 다음 재생
        - 인벤토리에 `nasal_cannula` 아이템이 있는지 확인
          ...
          - 있다면 다음 처리
            ...
            2. 순서 무관 상호 안내 처리 (2026-08-19 확정)
              - 비강 캐뉼라 적용과 산소유량계(oxyflowmeter) 설치는 어느 쪽이 먼저 일어나도 무방하다. 두
                조건이 모두 충족될 때까지, 아직 끝나지 않은 나머지 한 쪽만 아래 규칙으로 안내한다.
              - 지금 막 비강 캐뉼라가 적용됐고, 환자가 위치한 zone의 산소유량계가 아직 설치(활성화)되지
                않았다면
                - Dialogue
                  - Speaker: (플레이어 이름)
                  - Content: "(산소유량계를 설치하자.)"
                - 이후 산소유량계가 설치될 때까지 대기한다.
              - 반대로 산소유량계가 비강 캐뉼라보다 먼저 설치(활성화)된 상태로 지금 비강 캐뉼라가
                적용됐다면, 위 안내 없이 곧바로 다음(산소 연결 처리)으로 진행한다.
              - (참고) 비강 캐뉼라 적용 전에 산소유량계가 먼저 설치된 경우, "비강 캐뉼라 적용" Interaction이
                열려 있는 동안 아래를 먼저 재생한다.
                - Dialogue
                  - Speaker: (플레이어 이름)
                  - Content: "(비강 캐뉼라를 적용하자.)"
                - 이후 비강 캐뉼라가 실제로 적용되면 위 1번의 "(비강 캐뉼라를 환자에게 적용했다.)" 안내부터
                  이어서 진행한다.
              - 기술 노트: 산소유량계 "설치(활성화)" 판정은 처치 단계 크레딧용 신호(퀘스트 완료 처리에
                쓰이는 equipment_connected 신호, 비강 캐뉼라 적용 완료 이후에만 크레딧되도록 설계됨)가
                아니라, 산소유량계 오브젝트의 실제 설치 상태(IsAttached)를 그대로 반영하는 별도의 raw
                신호로 판정한다. 순서 판정에 크레딧 신호를 쓰면 비강 캐뉼라 적용 전에는 항상 "미설치"로만
                보여 반대 방향 안내가 나오지 않는다.
            3. 환자 Display State Descriptor에서 산소 공급 장치 오브젝트를 참조, 상태 플래그를 활성화
              - 기술 노트: 오브젝트가 Display되었다는 플래그와 산소 공급 장치가 연결되었다는 플래그를 분리해 고려, 관려해야함
            4. 환자가 위치한 CareZone의 flowmeter 오브젝트를 참조, 이 오브젝트의 하위에 위치한 Oxy connection point 오브젝트를 참조하여 환자의 oxy connection point와 연결되도록 처리
            5. 이후 퀘스트의 목표를 완료처리
            6. Dialogue
              - Speaker: (플레이어 이름)
              - Content: "산소 넣었습니다."
            7. 퀘스트 목표 표기를 "남성 환자 지혈하기"로 변경
