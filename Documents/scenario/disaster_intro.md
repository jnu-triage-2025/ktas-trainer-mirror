...
| D003_D | Dialogue | 준비실에 도착한 플레이어 D에게 "수액과 수액세트, 혈액과 혈액세트를 연결하십시오."라고 안내한다. 화면 텍스트로도 동일한 메세지를 표시한다. | CC_D_READY |
...
| D007 | Dialogue | 플레이어 B/C/D에게 "준비한 물품과 약물을 최종 점검 하십시오. 간호사 A의 분류가 완료될 때까지 준비합니다."라고 안내한다. 화면 텍스트로도 동일한 메세지를 표시한다. | CC_A_TRIAGE_DONE |
