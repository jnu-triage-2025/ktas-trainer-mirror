...
```
im-not-ai/                               # ktas-trainer의 하위 도구 (Tools/im-not-ai/)
├── CLAUDE.md                      # 본 파일 — 프로젝트 가이드
├── README.md                      # 사용 안내
├── LICENSE                        # MIT License
├── CONTRIBUTORS.md
├── scripts/
│   ├── prepare_monolith_input.py  # input shim — 텍스트 위생 + 정량 점수 + route_hint 산출 + 결합 입력
...
│   └── verify_gates.py            # 구조 수렴 게이트 (4축: 목표달성·대구 전멸·수치·golden)
├── tests/                         # pytest — metrics 단위 + 청킹 + 빌드 sync + golden 픽스처
│   ├── test_metrics.py · test_metrics_v2.py · test_chunking.py · test_golden.py 등
│   └── golden/                    # 윤문 품질 회귀 픽스처 + 결정적 채점기
├── agents/                        # 서브에이전트 9종
...
├── skills/                        # 스킬 3종
│   ├── humanize-korean/
│   │   ├── SKILL.md               # 오케스트레이터 (route_hint 3경로 분기·shim 배선)
│   │   └── references/
│   │       ├── quick-rules.md          # monolith 슬림 룰북
│   │       ├── ai-tell-taxonomy.md     # SSOT — 10대분류 × 활성 70 패턴
│   │       ├── rewriting-playbook.md   # 카테고리별 치환 레시피
│   │       ├── diagnosis-rules.md      # 진단 슬림 인덱스 (taxonomy에서 자동 생성)
│   │       ├── metrics.py · metrics_v2.py
│   │       ├── baseline.json · baseline_v2.json
│   │       ├── scholarship.md          # 학술 인용 외부 SSOT
│   │       └── web-service-spec.md     # 웹 확장 스펙 (옵션)
│   ├── humanize/SKILL.md          # 진입 스킬
│   └── humanize-redo/SKILL.md     # 후속 재실행 스킬
├── docs/
│   └── watermark-baseline-runbook.md
└── _workspace/                    # 런타임 산출물 (run_id별, gitignored)
    └── {YYYY-MM-DD-NNN}/
        ├── 01_input.txt · 00_metrics.json · 01_input_with_metrics.txt
        ├── final.md
        ├── 02_diagnosis.md             # (standard·heavy)
        └── final_pre_finalize.md · 09_finalize.json   # (heavy)
```
