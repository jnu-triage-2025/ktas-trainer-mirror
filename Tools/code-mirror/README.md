...
필터링 설정이 바뀌면 대상 이력도 달라집니다. 이때 도구는 중단하며, `--dry-run --rebuild`로 결과를 검토한 뒤 `--rebuild --push`를 명시해야 합니다. 이 중단은 전체 재변환이 비싼 작업이라 의도를 확인하기 위한 것이며, 대상 이력 보호와는 무관합니다. 대상에서 수동으로 삭제한 브랜치·태그는 다음 실행에서 원본에 아직 존재하면 다시 생성합니다. 반면 삭제된 원본 브랜치·태그는 대상 원격 저장소에서 자동 삭제하지 않습니다.
