...
export async function drivePatientBed(platform:Platform,instanceId:string,bedId:string,pointId:string,signal:AbortSignal,timeoutMs=60000,participantIds:string[]=[instanceId],via:number[][]=[],waypointSettleMs=0,finalTargetPosition?:number[]){
 ...
 try {
  while(performance.now()<deadline){
   ...
   const bed=beds[0];
   if(!finalTargetPosition&&bed.latchedPointId===pointId)return;
