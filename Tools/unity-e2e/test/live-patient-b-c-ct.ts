...
try {
 ...
 if(process.argv[4]==='b-care'){
  ...
  for(const [bedIndex,bed] of ['bed_b','bed_c'].entries() as ArrayIterator<[number,'bed_b'|'bed_c']>){
   ...
   try{
    await drivePatientBed(platform,controllers[0],bed,'ct:patient_target_pos_b',AbortSignal.timeout(180000),180000,controllers,bed==='bed_b'?ctRouteB:ctRouteC,250,'patientBed',ctTarget);
   }catch(error){
